...
const ProductImages = ({ itemImages }: { itemImages: any }) => {
  ...
  return (
    <div >
      <div className="h-[500px]  relative">
        <Image
          src={itemImages[index].image?.url}
          alt=""
          fill
          sizes="50vw"
      ...
      <div className="flex justify-between gap-4 mt-8">
        {itemImages.map((item: any, index: number) =>
          <div key={item._id} className="w-1/4 h-20 relative gap-4 mt8" onClick={() => setIndex(index)}>
            <Image
              src={item.image.url}
              alt=""
              fill
              sizes="30vw"
              className="object-fit rounded-md cursor-pointer"
            />
          </div>
        )
        }
